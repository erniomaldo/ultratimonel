...
> **Status:** Draft · **Updated:** 29 Jul 2026
...
| F-DS-05 | The status endpoint (`action=status`) SHALL read residual stderr from the dashboard subprocess before nulling its reference, and return both `exit_code` and `stderr_tail` (last 2000 chars) in the JSON response | MUST |
| F-DS-06 | On immediate post-start crash, the start handler SHALL return a `hint` field pointing to the log file path instead of an inline `stderr` string | MUST |
...
**AND** `_dashboard_proc` is set to `None` only after its stderr buffer has been read
